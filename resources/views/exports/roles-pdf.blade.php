...
<head>
    <meta charset="utf-8">
    <title>DAFTAR ROLE APLIKASI</title>
    <style>
        @page {
            size: A4;
        }
...
<body style="margin: 15mm; padding: ; font-family: 'Times New Roman', Times, serif; font-size: 12px; line-height: 1.2; color: #111;">
    <div class="layout-wrapper" style="padding: 8px 12px 12px;">
            ...
            <table style="width: 100%; border-collapse: collapse; border-spacing: 0;">
                <tr>
                    <td style="width: 90px; text-align: center; vertical-align: middle; border: 1px solid transparent;">
                        <img src="{{ public_path('logos/logo.svg') }}" alt="Logo Aceh Jaya" style="width: 75px; height: auto; display: block; margin: 0 auto;">
                    </td>
        ...
        <h1 class="section-title" style="font-size: 18px; font-weight: 700; margin: 18px 0 4px; text-align: center; text-transform: uppercase; letter-spacing: 0.6px;">Data Role Aplikasi</h1>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 8px;">
            <thead>
                <tr>
                    ...
                    <th style="border: 0.6px solid #555; padding: 6px 8px; background-color: #f3f3f3; font-weight: 700; text-align: center; font-size: 12px; text-transform: uppercase;">Nama Role</th>
                    <th style="border: 0.6px solid #555; padding: 6px 8px; background-color: #f3f3f3; font-weight: 700; text-align: center; font-size: 12px; text-transform: uppercase;">Guard Name</th>
                    <th style="border: 0.6px solid #555; padding: 6px 8px; background-color: #f3f3f3; font-weight: 700; text-align: center; font-size: 12px; text-transform: uppercase;">Permissions</th>
                    <th style="border: 0.6px solid #555; padding: 6px 8px; background-color: #f3f3f3; font-weight: 700; text-align: center; font-size: 12px; text-transform: uppercase;">Status</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr @if($loop->iteration % 2 === 0) style="background-color: #fafafa;" @endif>
                    ...
                    <td style="border: 0.6px solid #555; padding: 6px 8px; font-size: 11px; vertical-align: top;">{{ $role->name }}</td>
                    <td style="border: 0.6px solid #555; padding: 6px 8px; font-size: 11px; vertical-align: top;">{{ $role->guard_name }}</td>
                    <td style="border: 0.6px solid #555; padding: 6px 8px; font-size: 11px; vertical-align: top; text-transform: capitalize">{{ $role->permissions->pluck('name')->implode(', ') ?: 'Tidak ada permission' }}</td>
                    <td style="border: 0.6px solid #555; padding: 6px 8px; font-size: 11px; vertical-align: top;">{{ $role->status ? 'Aktif' : 'Tidak aktif' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
